...
def detection(frame):
    ...
    kps = [cv2.KeyPoint(x=f[0][0], y=f[0][1], size=20) for f in corners] # f[row][column] to access the coordinates stored in corners array
...
def matching(frame, ref_des, cur_des):
    ...
    for m,n in matches:
        if m.distance < 0.7 * n.distance:
            ...
            # Draw the trails for the matched points using lines
            cv2.line(frame, (u_train, v_train), (u_query, v_query), (0, 0, 255), 2)
